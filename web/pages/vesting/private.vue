      ...
      <div class="el__box my-4">
        ...
        <div class="progress__wrap mb-4" v-if="inVestingPool">
          ...
          <div class="d-flex justify-content-between mt-3">
            ...
            <p>
              {{ numberWithCommas(amountClaimed) }} /
              {{ numberWithCommas(dataVestingAccount[0]) }} ASC
            </p>
          </div>
        </div>
        ...
        <div class="el__wrap maxwidth-380">
          <ul class="el__list -investor -light mb-3">
            ...
            <li>
              ...
              <div class="li__val ms-auto">
                Lock 1st, 2nd month, vest 6.13% 3rd month, lock 4th month, from
                5th month,<br />
                vest 6.13% monthly.
              </div>
            </li>
...
import VestingPrivate from "~/plugins/contracts/services/VestingPrivate";
...
export default {
  data() {
    return {
      claimableMilestones: [
        1647621000, 1655569800, 1660840200, 1663518600, 1666110600, 1668789000,
        1671381000, 1674059400, 1676737800, 1679157000, 1681835400, 1684427400,
        1687105800, 1689697800, 1692376200, 1695054600,
      ],
      claimablePercents: [
        800, 613, 613, 613, 613, 613, 613, 613, 613, 613, 613, 613, 613, 613,
        613, 618,
      ],
      denominator: 10000,
      statusClaimData: [],
  ...
  methods: {
    async initData() {
      try {
        const dataVestingAccountBN = await VestingPrivate.callVestingOf(
          this.provider,
          this.account
        );
        ...
        this.isClaimedTGE = await VestingPrivate.callHasClaimedTGE(
          this.provider,
          this.account
        );
    ...
    async claim(type) {
      if (this.account) {
        try {
          ...
          if (type === "tge") {
            receipt = await VestingPrivate.callClaimTGE(this.provider);
          } else receipt = await VestingPrivate.callClaim(this.provider);
