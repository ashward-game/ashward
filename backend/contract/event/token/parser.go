...
func ParseLogApproval(contractAbi *abi.ABI, vLog *types.Log) (*LogApproval, error) {
	...
	evt.Spender = common.HexToAddress(vLog.Topics[2].Hex())
...
const LogTaxUpdatedName string = "TaxUpdated"
const LogTaxUpdatedSig string = "TaxUpdated(uint256)"

type LogTaxUpdated struct {
	Tax *big.Int
}

func ParseLogTaxUpdated(contractAbi *abi.ABI, vLog *types.Log) (*LogTaxUpdated, error) {
	var evt LogTaxUpdated
	if len(vLog.Data) > 0 {
		err := contractAbi.UnpackIntoInterface(&evt, LogTaxUpdatedName, vLog.Data)
		if err != nil {
			return nil, err
		}
	}
...
func (p *parserHandler) ParseHandle(ctx context.Context, vLog *types.Log) error {
	contractAbi, err := abi.JSON(strings.NewReader(string(token.ABI)))
	if err != nil {
		return err
	}
	...
	for k, v := range hashMap {
		if k == vLog.Topics[0].Hex() {
			...
			case LogTaxUpdatedName:
				evt, err := parser.(func(*abi.ABI, *types.Log) (*LogTaxUpdated, error))(&contractAbi, vLog)
				if err != nil {
					return err
				}
				return handler.(func(context.Context, *LogTaxUpdated) error)(ctx, evt)
			...
			default:
				return errors.New("token: event type is not supported")
			}
		}
	}
	return errors.New("token: topic is not supported")
}
...
func init() {
	...
	logParserMap[LogTaxUpdatedName] = ParseLogTaxUpdated
	...
	logHandlerMap[LogTaxUpdatedName] = HandleLogTaxUpdated
	...
	logTaxUpdatedSigHash := crypto.Keccak256Hash([]byte(LogTaxUpdatedSig))
	hashMap[logTaxUpdatedSigHash.Hex()] = LogTaxUpdatedName
